...
class CacheClient:
    def __init__(self, host="master", port=50051):
        ...
        self.memcached = pylibmc.Client(["memcached:11211"])  # Cambia a la dirección del servicio Memcached

    def get(self, key, simulated=False):
        start_time = time.time()  # Inicio del temporizador

        # Intenta obtener el valor del caché de Memcached
        ...
        if value is not None:
            ...
            if value:
                ...
                # Agregando la llave-valor al caché de Memcached
                self.memcached.set(key, value)
                
                elapsed_time = time.time() - start_time  # Calcula el tiempo transcurrido
                if simulated:
                    # Agregar el retraso al tiempo solo suma
                    elapsed_time += delay
    ...
    def simulate_searches(self, n_searches=100):
        ...
        print(f"\ttiempo total: {time_total:.2f} seconds")
    ...
    def simulate_distribucion_normal(self, n_simulations=100):
        ...
        for id_aleatorio in ids_aleatorios:
            ...
            count += 1
            print("\033[H\033[J")
            print(f"Searching : {count}/{n_simulations}")
            start_time = time.time()
            time_without_cache += 3 + 0.001  # Estimado de tiempo de búsqueda en JSON
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print("3. Simulate Distribucion normal")
        print("4. Exit")
        ...
        if choice == "1":
            ...
            n_simulations = int(input("Enter the number of simulations you want to run: "))
